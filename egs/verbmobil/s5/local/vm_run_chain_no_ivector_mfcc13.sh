...
num_threads_ubm=32
nnet3_affix=mfcc13       # affix for exp dirs, e.g. it was _cleaned in tedlium.

# Options which are not passed through to run_ivector_common.sh
affix=1x   #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
common_egs_dir=
reporting_email=
...
if ! cuda-compiled; then #todo: check if cli option
  cat <<EOF && exit 1
...
lat_dir=exp/chain${nnet3_affix}/${gmm}_${train_set}
...
# remove --feat.online-ivector-dir=$train_ivector_dir to not use ivectors
  ...
  for data in $test_sets; do
   ( 
      ...
      steps/nnet3/decode.sh \
        ...
        $tree_dir/graph data/${data} ${dir}/decode_${data_affix} || exit 1
...
# normal decoding.
## NO IVECTORS-NO ONLINE DECODING?
